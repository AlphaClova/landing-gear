...
import { RetrievedEvidenceSection } from './RetrievedEvidence'
...
export function PensionChat(props: PensionChatProps) {
  ...
  const renderCitations = (citations: Extract<ChatResponse, { type: 'result' }>['citations']) => citations.length > 0 && <section className="pension-answer-section pension-evidence" aria-labelledby="api-evidence-title"><h2 id="api-evidence-title">근거와 출처</h2><div className="evidence-list">{citations.map((citation) => <article className="evidence-item" key={citation.id}><div className="evidence-summary"><b>{citation.source || citation.documentId || '출처'}</b><strong>{citation.documentTitle}</strong>{citation.page !== null && <small>{citation.page}페이지</small>}{citation.excerpt && <p>{citation.excerpt}</p>}{citation.url && <a href={citation.url} target="_blank" rel="noreferrer">원문 확인<span className="sr-only">(새 창)</span></a>}</div></article>)}</div></section>
  ...
  return <section className="pension-slice" aria-labelledby="pension-title">
    ...
    {response && <div ref={resultRef} className="pension-conversation focus-target" tabIndex={-1} aria-label="답변 결과">
      ...
      {response.type === 'result' && !productModel && !resultModel && <><section className="pension-answer-section"><StatusBadge tone="navy">확인된 결과</StatusBadge><h2>상담 답변</h2>{response.conclusion.split('\n\n').map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{response.explanation && response.explanation !== response.conclusion && <p>{response.explanation}</p>}</section>{response.comparison && <section className="pension-answer-section pension-comparison"><h2>{response.comparison.title}</h2><div className="pension-comparison-table" role="table">{response.comparison.rows.map((row) => <div className="pension-comparison-row" role="row" key={row.id}><strong role="rowheader">{row.label}</strong><span role="cell">{row.optionA ?? '확인 필요'}</span><span role="cell">{row.optionB ?? '확인 필요'}</span></div>)}</div></section>}{response.retrievedContextView ? <RetrievedEvidenceSection view={response.retrievedContextView} /> : renderCitations(response.citations)}</>}

      {response.type === 'clarification' && <section className="pension-answer-section pension-state"><StatusBadge tone="amber">추가 정보 필요</StatusBadge><h2>추가로 확인할 내용</h2>{response.message && <p>{response.message}</p>}<ul>{response.requiredSlots.map((slot) => <li key={slot.key}><strong>{slot.label}</strong>{slot.reason && <p>{slot.reason}</p>}</li>)}</ul></section>}

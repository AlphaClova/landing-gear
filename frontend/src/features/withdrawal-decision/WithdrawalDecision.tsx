...
export function WithdrawalDecision() {
  ...
  const submit = async (event?: FormEvent) => {
    ...
    catch (error) { if (!(error instanceof DOMException && error.name === 'AbortError')) setViewModel({ ...emptyErrorViewModel, input }) }
  ...
  return <section className="withdrawal-slice" aria-labelledby="withdrawal-title">
    ...
    <details className="withdrawal-input-panel" open={!hasResult || undefined}><summary>{hasResult ? '입력 조건 확인·수정' : '비교 조건 입력'}</summary><form onSubmit={submit} noValidate>
      {viewModel?.status === 'needs_input' && <div className="withdrawal-input-alert" aria-live="polite"><strong>{viewModel.summary}</strong><p>부족한 조건: {viewModel.missingFields.map((field) => fieldLabels[field]).join(', ')}</p></div>}
      <div className="withdrawal-fields">
        <label className={missing.has('retirementBenefitAmount') ? 'field-error' : ''}><span>퇴직급여 예상액</span><input type="number" min="0" inputMode="numeric" value={input.retirementBenefitAmount ?? ''} onChange={(e) => update('retirementBenefitAmount', inputNumber(e.target.value))} aria-invalid={missing.has('retirementBenefitAmount')} /><small>{input.retirementBenefitAmount === null ? '원 단위로 입력' : `${won.format(input.retirementBenefitAmount)}원`}</small></label>
        <label className={missing.has('currentAge') ? 'field-error' : ''}><span>현재 나이</span><input type="number" min="0" value={input.currentAge ?? ''} onChange={(e) => update('currentAge', inputNumber(e.target.value))} aria-invalid={missing.has('currentAge')} /><small>세</small></label>
        <label className={missing.has('pensionStartAge') ? 'field-error' : ''}><span>연금 수령 시작 나이</span><input type="number" min="0" value={input.pensionStartAge ?? ''} onChange={(e) => update('pensionStartAge', inputNumber(e.target.value))} aria-invalid={missing.has('pensionStartAge')} /><small>세</small></label>
      ...
      <div className="withdrawal-form-actions">{isMockWithdrawalMode && <button type="button" className="secondary-button" onClick={() => { setInput(exampleWithdrawalInput); setAdditionalOpen(true) }}>예시 조건 불러오기</button>}<button type="submit" className="button">{loading ? '비교 중' : hasResult ? '조건을 바꿔 다시 비교' : '수령 방식 비교하기'}</button></div>
    </form></details>

    {loading && <section className="withdrawal-loading" aria-live="polite" aria-busy="true"><div aria-hidden="true"><i /><i /><i /></div><strong>비교 결과를 준비하고 있습니다</strong><button type="button" onClick={cancel}>요청 취소</button></section>}
    {cancelled && !loading && <p className="withdrawal-notice" role="status">요청을 취소했습니다. 입력한 조건은 유지됩니다.</p>}
    {viewModel && !loading && <WithdrawalResult viewModel={viewModel} onRetry={submit} />}
  </section>
}
